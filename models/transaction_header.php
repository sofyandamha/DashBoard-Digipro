...
class Transaction_Header extends Base_Model
{
    ...
    function get_all_by_date($shop_id, $conditions = array(), $limit=false, $offset=false)
    {
        $this->db->from($this->table_name);
        $this->db->where('shop_id', $shop_id);
        ...
        if (isset($conditions['start_date']) && isset($conditions['end_date'])) {
            if($conditions['start_date'] != "" && $conditions['end_date'] != ""){
                //echo "start_date = ".strtotime($conditions['start_date'])." end_date ".strtotime($conditions['end_date']);
                $this->db->where('added BETWEEN "'. date('Y-m-d H:i:s', strtotime($conditions['start_date'])).
                    '" AND "'.
                    date('Y-m-d H:i:s', strtotime($conditions['end_date'])).'"');
            }
        }
    ...
    function get_all_by_shop($limit=false, $offset=false)
    {
        $this->db->from($this->table_name);
