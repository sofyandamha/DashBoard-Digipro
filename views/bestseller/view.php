				...
				<?php
					...
					if(isset($items) && count($items->result())>0):
						foreach($items->result() as $item) {
                                ...
                                if ($item->name === $key) {
                                   // echo "item name : " . $item->name . "<br>";
                                    ...
                                    <tr>
                                        ...
                                        if (!$this->session->userdata('is_shop_admin')) {
                                            if (in_array('edit', $allowed_accesses)):?>
                                                <td><a href='<?php echo site_url("bestseller/edit/" . $item->id); ?>'><i
                                        ...
                                        if (!$this->session->userdata('is_shop_admin')) {
                                            if (in_array('delete', $allowed_accesses)):?>
                                                <td><a href='<?php echo site_url("bestseller/delete/" . $item->id); ?>'><i
                                    ...
                                    //echo "else 1 item name : " . $item->name . "<br>";
                                    ?>
                                    <tr>
                                        <td><?php echo ++$count; ?></td>
                                        <td><?php echo $item->name; ?></td>
                                            ...
                                            if (in_array('publish', $allowed_accesses)):?>
                                                <td>
                                                    <?php if ($item->is_published == 1): ?>
                                                        ...
                                                        <button class="btn btn-sm btn-primary unpublish"
                                                                itemId='<?php echo $item->item_id; ?>'>Yes
                                                        </button>

                                                    <?php else: ?>

                                                        <button class="btn btn-sm btn-danger publish"
                                                                itemId='<?php echo $item->item_id; ?>'>No
                                                        </button>
                           ...
                           // echo "else * item name : " . $item->name . "<br>";
                        }
					else:
                         echo "else 2";
                        foreach($items->result() as $item) {
                            echo "else 2 item name : " . $item->name . "<br>";
                        }
				?>
						<tr>
            ...
            // analyticAll($id);
            //echo $pag."<br> cat : ".$categories."<br> shop id : ".$shop_id;
            //echo "cat name :". $all->cat_name;
/*
           echo "All <br><pre>";
           print_r($transaction);
...
*/
			...
			$(document).ready(function(){
				$(document).delegate('.publish','click',function(){
					...
					$.ajax({
						url: '<?php echo site_url('bestseller/publish_view');?>/'+id,
						method:'GET',
				...
				$(document).delegate('.unpublish','click',function(){
					...
					$.ajax({
                        url: '<?php echo site_url('bestseller/unpublish_view');?>/' + id,
                        method: 'GET',
                        success: function (msg) {
                            if (msg == 'true') {
                                console.log("unpublish ok "+id);
