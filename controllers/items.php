...
//        $content['content'] = $this->load->view('items/qr_code', $data, true);
//
//        $this->load_template($content);
    }
	...
	function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD')=='POST') {
			...
			if ($this->item->save($item_data)) {
				$this->session->set_flashdata('success','Object is successfully added.');
			} else {
				$this->session->set_flashdata('error','Database error occured.Please contact your system administrator.');
			}
			...
			if ($action == "gallery") {
                ...
                redirect(site_url('items/qr_code/'.$item_data['id']));
            }
			else {
				redirect(site_url('items'));
			}
		...
		$cat_count = $this->category->count_all();
