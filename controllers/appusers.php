...
class Appusers extends Main
{
	function __construct()
	{
        ...
        $this->load->model ('appuser', 'customers');
	}
...
//        echo "</pre>";
	...
	function ajax_list()
    {
        $list = $this->appuser->get_datatables();
        ...
        foreach ($list as $customers) {
            ...
            $row[] = $customers->email;
            $row[] = $customers->phone;
            $str = chunk_split($customers->id_card, 4, ' ');
            $card = substr_replace($str, str_repeat("x", 4), 10, 4);
            $row[] = $card;
            $row[] = $customers->card_type;
		    ...
		    {
		         $row[] = $this->appuser->ago($customers->co);
		    }
		    
            $row[] = date("d-m-Y", strtotime($customers->added));
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->appuser->count_all_sort(),
            "recordsFiltered" => $this->appuser->count_filtered(),
            "data" => $data,
        );
