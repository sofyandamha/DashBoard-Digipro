					...
					<div class="row mb-2">
						...
						<div class="col-sm-6">
							...
							<ol class="breadcrumb float-sm-right">
								...
								<li class="breadcrumb-item active"><?php echo $this->lang->line('cat_list_label') ?></li>
							</ol>
						</div>
					</div>
				</div><!-- /.container-fluid -->
					...
					<table class="table table-striped table-bordered">
						...
						?>
								<tr>
									...
									if (!$this->session->userdata('is_shop_admin')) {
										if (in_array('edit', $allowed_accesses)) : ?>
											<td><a href='<?php echo site_url("categories/edit/" . $category->id); ?>'><i class='fas fa-edit'></i></a></td>
										<?php endif;
									} else { ?>
										<td><a href='<?php echo site_url("categories/edit/" . $category->id); ?>'><i class='fas fa-edit'></i></a></td>
									<?php } ?>
									...
									if (!$this->session->userdata('is_shop_admin')) {
										if (in_array('delete', $allowed_accesses)) : ?>
											<td><a class='btn-delete' data-toggle="modal" data-target="#myModal" id="<?php echo $category->id; ?>"><i class='fa fa-trash'></i></a></td>
										<?php endif;
									} else { ?>
										<td><a class='btn-delete' data-toggle="modal" data-target="#myModal" id="<?php echo $category->id; ?>"><i class='fa fa-trash'></i></a></td>
									<?php } ?>
										...
										if (in_array('publish', $allowed_accesses)) : ?>
											<td>
												<?php if ($category->is_published == 1) : ?>
													<button class="btn btn-sm btn-primary unpublish" catId='<?php echo $category->id; ?>'>Yes</button>
												<?php else : ?>
													<button class="btn btn-sm btn-danger publish" catId='<?php echo $category->id; ?>'>No</button><?php endif; ?>
											</td>
										<?php endif;
									} else { ?>
										<td>
