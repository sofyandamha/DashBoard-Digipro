			...
			echo form_open(site_url("bestseller/edit/".$item->id), $attributes);
			...
			?>
				...
				<a class="btn btn-primary" href="<?php echo site_url('bestseller/gallery/'.$item->id);?>"><?php echo $this->lang->line('goto_gallery_button')?></a>
				<a href="<?php echo site_url('bestseller');?>" class="btn btn-primary"><?php echo $this->lang->line('cancel_button')?></a>
			</form>
			</div>
			<script>
				$(document).ready(function(){
					$('#item-form').validate({
						rules:{
							name:{
								...
								minlength: 4,
								remote: {
									url: '<?php echo site_url("bestseller/exists/".$item->id);?>',
								  	type: "GET",
								  	data: {
								  		name: function () {
								  			return $('#name').val();
					...
					$('#cat_id').change(function(){
						var catId = $(this).val();
						$.ajax({
							url: '<?php echo site_url('bestseller/get_sub_cats');?>/'+catId,
							method: 'GET',
							dataType: 'JSON',
