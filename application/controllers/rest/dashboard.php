...
class dashboard extends REST_Controller
{
	...
	function acceptNotification_post()
	{
		$datas = array(
			'appuser_id' => $this->post('appuser_id'),
			'notification_id' => $this->post('notification_id'),
			'access_date' => date("Y-m-d H:m:s")
		);
		if($this->mk_notification_historys->updateIsRead($this->post('notification_id'),$this->post('appuser_id'))){
			if($this->mk_notification_contents->updateReachUser($this->post('notification_id'))){
				$this->response(array(
					'status'=>'success',
					'data'	=> true
				));
	...
	function register_post()
	{
		...
		if ($platform_name == "android") {
			...
			if ( $this->gcm_token->exists( $user_data )) {
				...
				$this->gcm_token->update($user_data,"android");
				$this->response(array(
					'status'=>'success',
