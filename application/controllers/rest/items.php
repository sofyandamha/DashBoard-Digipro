...
class Items extends REST_Controller
{
	function get_get()
	{
		...
		if (!$item_id && !$sub_cat_id) {
			$this->response(array('error' => array('message' => 'require_item_id_or_sub_cat_id test')));
		}
	...
	function get_items($shop_id, $sub_cat_id)
	{
		...
		*/
		...
		if (!$all) {
			$items = $this->item->get_all_by_sub_cat($sub_cat_id, $keyword, 8)->result();
	...
	function review_post()
	{
		...
		if ($data == null) {
			$this->response(array('error' => array('message' => 'invalid_json d')));
		}
        ...
        if(array_key_exists('rating', $data)){
            ...
            $this->rating->save_from_review($data,$item_id);
            $this->review->save_rating($data);
...
// edit maliki
    ...
    public function popular_items_get()
    {
        $count = $this->get('count');
        $from = $this->get('from');
	...
	function poin_post()
	{
		...
		$touch  = array('item_id' =>$data['item_id'],'appuser_id' =>$data['appuser_id'] );
		// print_r($touch);
		// die();
		...
		// print_r($data);
		$this->rating->save($data);
		// print_r(array('item_id' =>  $data['item_id']));
		$this->touch->save($touch);
		$this->response(array(
			'status'=>'success')
		);
    ...
    function bestseller_get(){
        ...
        $item_all = $this->best_seller->get_all($shop_id);
        $items_id=array();
        $i=0;
        $item=array();
        $images=array();
        //echo count($item_all->result());
        //print_r($item_all->result());
        foreach ($item_all->result() as $key ){
            if($i<$limit){
                $item[]= $this->best_seller->get_info($item_all->result()[$i]->item_id,40,0);
                //$item->id[]=$item_all->result()[$i]->item_id;
                $items_id[]=$item_all->result()[$i]->item_id;
                //echo "<br>".$item_all->result()[$i]->item_id;
                $item[$i]->image= $this->image->get_all_by_type($item_all->result()[$i]->item_id,"item")->result();
                //echo "--".$item_all->result()[$i]->item_id;
...
//            //echo "-val-".$val."<br>";
//            $images[]= $this->image->get_all_by_type($val,"item")->result();
//        }
//        $item->images=$images;
//        echo "<pre>";
//        print_r($item);
