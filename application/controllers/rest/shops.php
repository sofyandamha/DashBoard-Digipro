...
class Shops extends REST_Controller
{
	function get_get()
	{
		...
		if ($id) {
			...
			$shop->about = html_entity_decode($shop->about);	
	...
	function items_list_by_sub_cat_get()
	{
		$all = $this->get('item');
		...
		if (!$all) {
			$items = $this->item->get_all_by_sub_cat($sub_cat_id, $keyword, 0)->result();
		} else {
			if ($count && $from) {
