		...
		$pdf = new FPDF('l','mm','A4');
		 ...
		 if ($this->input->post('generate')) {
			...
			if ($file_format == "pdf") {
				...
				$pdf->Cell(250,7,'Report Customer',0,1,'C');
				 $pdf->Cell(10,7,'',0,1);
				 $pdf->SetFont('Arial','B',10);
				 $pdf->Cell(50,6,'Username',1,0);
				 $pdf->Cell(75,6,'Email',1,0);
				 $pdf->Cell(75,6,'Phone',1,0);
				 $pdf->Cell(50,6,'Card',1,1);

				 $pdf->SetFont('Arial','',10);

				for ($i = 0; $i < count($data['appusers']) ; $i++) {
					 $pdf->Cell(50,6,$data['appusers'][$i]['username'],1,0);
					 $pdf->Cell(75,6,$data['appusers'][$i]['email'],1,0);
					 $pdf->Cell(75,6,$data['appusers'][$i]['phone'],1,0);
					 $pdf->Cell(50,6,$data['appusers'][$i]['id_card'],1,1);
				 }
				...
				if($id==cs){
					$object = new PHPExcel();
					  ...
					  $table_columns = array("Name", "Email","Phone", "Card");
					  ...
					  foreach($data['appusers'] as $row){
					    ...
					    $object->getActiveSheet()->setCellValueByColumnAndRow(3, $excel_row, $row['id_card']);
					    $excel_row++;
